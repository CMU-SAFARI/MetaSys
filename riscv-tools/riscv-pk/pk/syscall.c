...
/*
void sys_atom_create(int size)
{
    printk("[Atom system call] Size %d\n", size);
...
static int sys_crt_atom(unsigned int atomID, unsigned int offset, long attribute, unsigned int mapMultiple, int a4, int a5, int n)
{
  ...
  switch(attribute_size){
    ...
    case 1:
        total_attribute = a1;
    case 2:
        total_attribute = a1 | a2 ;
    case 3:
        total_attribute = a1 | a2 | a3;
  }
  */
  if(!mapMultiple)
  {
    ...
    //printk("[KERN] Create multiple atoms with attribute = %d and id = %d \n",attribute,atomID);
    unsigned int* attributes = (unsigned int*)attribute;      
    asm volatile ("fatom_select %0, %1\n\t"
        :: "r" (atomID), "r" (atomID)
      );
    for (size_t __i = 0 ; __i < offset ; __i++)
    {
  ...
  //printk("Create single atom syscall\n");
  return 0;
}
...
long do_syscall(long a0, long a1, long a2, long a3, long a4, long a5, unsigned long n)
{
  ...
  //  [-OLD_SYSCALL_THRESHOLD + SYS_atom_create] = sys_atom_create // atom_create_system_call
  };
