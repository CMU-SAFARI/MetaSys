...
//// ------------------------------------------------------------------------------------------------------------------ //
//// Mehrshad Modification
//class ALBPTWIO(implicit p: Parameters) extends CoreBundle()(p)
//  with HasCoreParameters {
//  val req = Decoupled(new PTWReq)
//  val resp = Valid(new PTWResp).flip
//  val ptbr = new PTBR().asInput
...
class PTW(n: Int)(implicit edge: TLEdgeOut, p: Parameters) extends CoreModule()(p) {
  ...
  // Cannot easily apply backpressure here as it breaks everything
  io.mem.req.valid := state === s_req && !l2_valid //&& !io.snoop.bp_q_full // apply backpressure
  ...
  io.snoop.valid := state === s_req && !l2_valid && !r_req.isLookup //&& !io.snoop.bp_q_full
  io.snoop.physical := Bool(true)
  io.snoop.cmd  := M_XRD
  io.snoop.typ  := log2Ceil(xLen/8)
  io.snoop.addr := pte_addr
  io.snoop.s1_kill := s1_kill || l2_hit
  io.snoop.s2_nack := io.mem.s2_nack
  io.snoop.s2_xcpt := io.mem.s2_xcpt


  when(state === s_req && !l2_valid && !io.snoop.bp_q_full && r_req.isLookup){
