...
trait ScalarOpConstants {
  val MT_SZ = 3 // This should not be changed,
                // it will mess with the instruction encodings
  def MT_X  = BitPat("b???")
  def MT_B  = UInt("b000")
  def MT_H  = UInt("b001")
  def MT_W  = UInt("b010")
...
trait MemoryOpConstants {
  ...
  def M_FILL    = UInt("b10101") // Fill cache with PAT address
  ...
  def isRead(cmd: UInt) = cmd === M_XRD || cmd === M_XLR || cmd === M_XSC || isAMO(cmd) || isFill(cmd)
  def isWrite(cmd: UInt) = cmd === M_XWR || cmd === M_PWR || cmd === M_XSC || isAMO(cmd)
  def isWriteIntent(cmd: UInt) = isWrite(cmd) || cmd === M_PFW || cmd === M_XLR
  def isFill(cmd: UInt) = cmd === M_FILL
}
