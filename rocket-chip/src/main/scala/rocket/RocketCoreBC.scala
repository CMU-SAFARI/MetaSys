...
class RocketBC(implicit p: Parameters) extends CoreModule()(p)
  ...
  val perfEvents = new EventSets(Seq(
    ...
    new EventSet((mask, hits) => (mask & hits).orR, Seq(
      ("Total requests", () => io.rocc.core_snoop.dbg4),//id_ex_hazard && ex_ctrl.mem || id_mem_hazard && mem_ctrl.mem || id_wb_hazard && wb_ctrl.mem),
      ("long-latency interlock", () => id_sboard_hazard),
      ("csr interlock", () => replay_wb),
      ("I$ blocked", () => ctrl_stalld),
    ...
    new EventSet((mask, hits) => (mask & hits).orR, Seq(
      ...
      ("DBG4", () => false.B),//io.rocc.core_snoop.dbg4),
      ("DBG5", () => io.dmem.perf.release)))
    ))
  ...
  ex_reg_xcpt_interrupt := (!take_pc && ibuf.io.inst(0).valid && csr.io.interrupt) || (io.rocc.interrupt)
  ...
  println("BC Core being generated")

  // replay inst in ex stage?
  val ex_pc_valid = ex_reg_valid || ex_reg_replay || ex_reg_xcpt_interrupt
  val wb_dcache_miss = wb_ctrl.mem && !io.dmem.resp.valid
  val replay_ex_structural = ex_ctrl.mem && !io.dmem.req.ready ||
                             ex_ctrl.mem && io.rocc.core_snoop.bp_q_full || // replay when we have a mem. inst. and lookup q. is full
  ...
  val replay_wb_rocc = wb_reg_valid && wb_ctrl.rocc && (!io.rocc.cmd.ready || io.rocc.busy)
  ...
  val stall_until_flushed = id_ctrl.rocc && (new RoCCInstruction().fromBits(id_inst(0)).funct === 20.U) &&
                              (ex_reg_valid || mem_reg_valid || wb_reg_valid)
  val stall_after_mem     = !bc_resolved && ex_ctrl.mem && ex_reg_valid
  ctrl_stalld :=
    id_ex_hazard || id_mem_hazard || id_wb_hazard || id_sboard_hazard ||
    csr.io.singleStep && (ex_reg_valid || mem_reg_valid || wb_reg_valid) ||
    id_ctrl.fp && id_stall_fpu ||
    id_ctrl.mem && io.rocc.core_snoop.bp_q_full ||// reduce activity when lookup q fills up
  ...
  io.dmem.req.bits.isLookup := false.B
  ...
  io.rocc.core_snoop.physical   := Bool(false)

  when(io.rocc.interrupt)
  {
    printf("RoCC interrupted core\n")
  }

  io.rocc.cmd.valid := wb_reg_valid && wb_ctrl.rocc && !replay_wb
  ...
  val mshr_busy_ctr = RegInit(UInt(0,64.W))
  mshr_busy_ctr := mshr_busy_ctr + PopCount(Vec(io.dmem.perf.mshr_busy.toBools))
  ...
  csr.io.counters(6).inc := RegNext(PopCount(Vec(io.dmem.perf.mshr_busy.toBools)))
