...
//  val s1_isfill = isFill(s1_req.cmd)
  ...
  when (!tlb.io.req.ready && !tlb.io.ptw.resp.valid && !io.cpu.req.bits.phys) { io.cpu.req.ready := false }  // !(io.cpu.req.bits.cmd === M_ALB_MAP) &&
  when (s1_valid && (s1_readwrite || s1_sfence) && tlb.io.resp.miss) { s1_nack := true } // !(io.cpu.req.bits.cmd === M_ALB_MAP) &&
  ...
  //val s2_isfill = isFill(s2_req.cmd)
  ...
  val pstore1_isLookup = RegEnable(s1_req.isLookup, s1_valid_not_nacked && s1_read)
  ...
  val pstore2_isLookup = RegEnable(Mux(s2_correct, s2_req.isLookup, pstore1_isLookup), advance_pstore1)
  ...
  io.cpu.resp.bits.paddr := s1_paddr
  ...
  // AMOs
  if (usingRMW) {
    // when xLen < coreDataBits (e.g. RV32D), this AMO ALU is wider than necessary
  ...
  io.cpu.perf.access  := s2_valid && s2_hit_valid && !s2_data_error
