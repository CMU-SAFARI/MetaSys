...
class HellaCacheArbiter(n: Int)(implicit p: Parameters) extends Module
{
  ...
  if (n == 1) {
    ...
    for (i <- 1 until n){
      ...
      /*
      when(io.requestor(i).req.ready && io.requestor(i).req.valid)
      {
        printf("I am an arbiter, and I let my %d(st-nd-rd-th) requestor request!\n", UInt(i))
      }
      */

    }

    // Requestor i=2 is currently the prefetcher
    for (i <- n-1 to 0 by -1) {
      ...
      def connect_s0() = {
        ...
        io.mem.req.bits.isLookup := req.bits.isLookup
        s1_id := UInt(i)
      }
      def connect_s1() = {
    ...
    for (i <- 0 until n) {
      ...
      /*
      when( resp.valid && UInt(i) === UInt(2) && tag_hit ){
        printf("Pref Resp\n")
      }
      */
