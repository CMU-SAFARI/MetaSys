 ...
 */
 ...
 // Can this be not ready for a long while, at the same time not requesting
 // anything?
class SimpleHellaCacheIFReplayQueue(depth: Int)
  ...
  when(!io.replay.valid && !io.req.ready)
  {
    printf("HellaCacheIF: replaying: %d\n", replaying)
  }
  
  // Only one replay outstanding at a time
  when (io.replay.fire()) { replaying := Bool(true) /*printf("SimpleHellaCacheIF fired a request\n")*/ }
...
class SimpleHellaCacheIF(implicit p: Parameters) extends Module
{
  ...
  val s2_req    = RegNext(RegNext(req_arb.io.out))
  ...
  when(io.requestor.req.fire() && isPrefetch(io.requestor.req.bits.cmd)){
    printf("Prefetcher requested a prefetch\n")
  }

  when(io.cache.req.fire() && isPrefetch(io.cache.req.bits.cmd))
  {
    printf("Cache IF requested a prefetch\n")
  }
  
  
  when(RegNext(RegNext(io.cache.req.fire())) && io.cache.s2_xcpt.asUInt().orR()) {
    printf("XCPT addr: 0x%x s2_xcpt : %d  ", s2_req.bits.addr, io.cache.s2_xcpt.asUInt())
    printf("ma.st : %d  ", io.cache.s2_xcpt.ma.st)
    printf("ma.ld : %d  ", io.cache.s2_xcpt.ma.ld)
    printf("pf.st : %d  ", io.cache.s2_xcpt.pf.st)
    printf("pf.ld : %d  ", io.cache.s2_xcpt.pf.ld)
    printf("ae.st : %d  ", io.cache.s2_xcpt.ae.st)
    printf("ae.ld : %d\n", io.cache.s2_xcpt.ae.ld)
  }

  // ROCC accesses cannot cause exceptions?

//  assert(!RegNext(RegNext(io.cache.req.fire())) || !io.cache.s2_xcpt.asUInt.orR,
//         "SimpleHellaCacheIF exception")
//
//  assert( !io.cache.s2_xcpt.asUInt.orR,
//    "SimpleHellaCacheIF exception")
}
