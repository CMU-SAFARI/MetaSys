...
class IOMSHR(id: Int)(implicit edge: TLEdgeOut, p: Parameters) extends L1HellaCacheModule()(p) {
  ...
  var a_size = mtSize(req.typ)
  // a_size is 2 bits wide, we need to widen it if we want wider cache rows
  if(beatBytes == 64)
    a_size = Cat(UInt(0,(log2Up(log2Up(beatBytes))-2).W) , mtSize(req.typ))
  ...
  io.mem_access.bits := Mux(isAMO(req.cmd), atomics, Mux(isRead(req.cmd) || isPrefetch(req.cmd), get, put))

  io.replay_next := ((state === s_mem_ack) || (io.resp.valid && !io.resp.ready)) && !req.isLookup
  /*
  when(req.isLookup && ((state === s_mem_ack) || (io.resp.valid && !io.resp.ready)))
  {
    printf("Did not set replay_next because no structural hazard\n")
  }
  when(req.isLookup)
  {
    printf("Sanity check\n")
  ...
  */
  ...
  when (io.mem_access.fire()) {
    state := s_mem_ack
    /*
    when(isPrefetch(req.cmd))
    {
      printf("MSHR received prefetch addr:%x\n", req.addr)
    }
    */
  }
...
class MSHR(id: Int)(implicit edge: TLEdgeOut, p: Parameters) extends L1HellaCacheModule()(p) {
  ...
  io.refill.wasLookup := req.isLookup
  io.tag := req_tag
  ...
  val a_size = Cat(UInt(0,(log2Up(log2Up(beatBytes))-2).W) , mtSize(req.typ))

  println("Will slowdown cache bandwidth by " + bwSlowdown)
  if(bwSlowdown>1)
  {
    val very_simple_ctr_to_scale_bw = RegInit(UInt(0, 6.W))
    val ctr_valid                   = very_simple_ctr_to_scale_bw === (bwSlowdown-1).U
    when(ctr_valid){
      very_simple_ctr_to_scale_bw    := 0.U
    }.otherwise{
      very_simple_ctr_to_scale_bw    := very_simple_ctr_to_scale_bw + 1
    }

    io.mem_acquire.valid := state === s_refill_req && grantackq.io.enq.ready && ctr_valid
  }else{
    io.mem_acquire.valid := state === s_refill_req && grantackq.io.enq.ready
  }
  
  io.mem_acquire.bits := Mux(req.isLookup, 
                                edge.Get(UInt(id), req.addr, a_size)._2,
                                edge.AcquireBlock(
                                fromSource = UInt(id),
...
class MSHRFile(implicit edge: TLEdgeOut, p: Parameters) extends L1HellaCacheModule()(p) {
  val io = new Bundle {
    ...
    val mshr_busy = UInt(width = cfg.nMSHRs).asOutput
    val rnc = Bool(OUTPUT) // cpu/ptw request is not cacheable
  }

  val mshr_busy_vec = Wire(Vec(Seq.fill(cfg.nMSHRs)(false.B)))
  io.mshr_busy := mshr_busy_vec.asUInt

  // determine if the request is cacheable or not
  val cacheable = edge.manager.supportsAcquireBFast(io.req.bits.addr, lgCacheBlockBytes) //&& !io.req.bits.isLookup

  io.rnc := io.req.valid && !cacheable && !io.req.bits.isLookup
  ...
  val mshrs = (0 until cfg.nMSHRs) map { i =>
    ...
    mshr_busy_vec(i) := !mshr.io.req_pri_rdy
...
class PrivateAttributeTable (implicit p: Parameters) extends Module {
  val io = IO(new Bundle {
    ...
    val wen   =  Bool(INPUT)
    val waddr = UInt(INPUT,8)
  })
  val tableSize = 255
  val pat = RegInit(Vec(Seq.fill(tableSize)(0.U(32.W))))
  def access(addr: UInt) = pat(~addr)

  when(io.wen) { // If write enable and activate -> write activation
    access(io.waddr) := io.wdata
  }
...
class DataArray(implicit p: Parameters) extends L1HellaCacheModule()(p) {
  ...
  println("----------------------------NBDCache-----------------------------")
  println("Refill Cycles: " + refillCycles)
  println("Row Bits: " + encRowBits)
  println("Row Words: " + rowWords)
  println("Do Narrow Read: " + doNarrowRead)
  println("-----------------------------------------------------------------")
...
class NonBlockingDCacheModule(outer: NonBlockingDCache) extends HellaCacheModule(outer) {
  ...
  val mshr_busy = RegNext(mshrs.io.mshr_busy)
  ...
  val s1_replay_lookup = Reg(init=Bool(false))
  ...
  pat.io.wen := s1_fill
  pat.io.waddr := s1_req.tag
  pat.io.wdata := s1_req.data

 when(s1_fill && s1_valid)
  {
      printf("============ Hello from NBD cache ==================\n")
      printf("MESSAGE TO FILL PAT\n")
      printf("s1_req.addr = %d\n", s1_req.addr)
      printf("s1_req.data = %d\n", io.cpu.s1_data.data)
      printf("s1_isread = %d \n", s1_read )
      printf("=====================================================\n")
  }
  ...
  io.ptw <> dtlb.io.ptw
  dtlb.io.req.valid := s1_valid && !io.cpu.s1_kill && (s1_readwrite || s1_sfence) && !(s1_replay && s1_replay_lookup)
  ...
  dtlb.io.req.bits.isLookup := s1_req.isLookup || (s1_replay && s1_replay_lookup)
  when (!dtlb.io.req.ready && !io.cpu.req.bits.phys) { io.cpu.req.ready := Bool(false) }

  when(isPrefetch(s1_req.cmd) && s1_valid) {printf("--- D$ PRF --- addr:0x%x tlb_miss:%d\n", s1_req.addr, dtlb.io.req.valid && dtlb.io.resp.miss)}
  ...
  when (s1_clk_en) {
    s2_req.isLookup := s1_req.isLookup
  ...
  val (s2_has_permission, s2_dbg_req, s2_new_hit_state) = s2_hit_state.onAccess(s2_req.cmd)
  val s2_hit = (s2_tag_match && s2_has_permission && s2_hit_state === s2_new_hit_state) || RegNext(s1_replay_lookup && s1_replay)
  ...
  mshrs.io.req.bits.atomLookup := s2_req.isLookup
  ...
  s1_replay_lookup := mshrs.io.replay.bits.isLookup
  /*
  when(s1_replay_lookup && s1_replay){
    printf("Sanity check is lookup\n")
  }
  */
  ...
  /*
  when(metaWriteArb.io.in(0).valid)
  {
    printf("-- D$-MetaFill -- idx: 0x%x tag: 0x%x way_en: 0x%x\n",
      metaWriteArb.io.in(0).bits.idx, metaWriteArb.io.in(0).bits.data.tag,
      metaWriteArb.io.in(0).bits.way_en)
  }
  */
  ...
  writeArb.io.in(1).valid := tl_out.d.valid && grant_has_data && !mshrs.io.refill.wasLookup &&
  ...
  val lookup_buffer = Reg(UInt(encRowBits.W))
  when (tl_out.d.valid && grant_has_data && mshrs.io.refill.wasLookup){
    lookup_buffer := tl_out.d.bits.data(encRowBits-1,0)
  }

  /*
  when(writeArb.io.in(1).valid)
  {
    printf("-- D$-Fill -- addr: 0x%x, way: 0x%x\n", writeArb.io.in(1).bits.addr, writeArb.io.in(1).bits.way_en)
  }
  */
  ...
  val loadgen_lookup = new LoadGen(s2_req.typ, mtSigned(s2_req.typ), s2_req.addr, lookup_buffer, false.B, wordBytes)
  ...
  val s2_nack_victim = s2_hit && mshrs.io.secondary_miss && !RegNext(s1_replay_lookup && s1_replay)
  ...
  cache_resp.bits.data := Mux(RegNext(s1_replay_lookup && s1_replay), loadgen_lookup.data, loadgen.data | s2_sc_fail)
  ...
  when(s2_valid_masked && !s2_req.isLookup)
  {
    when(~s2_hit){
      printf("--- D$-DemandMiss --- vaddr: 0x%x paddr: 0x%x s2_hit: %d replay: %d is_write: %d, s2_tag_match: %d s2_has_permission: %d s2_hit_state === s2_new_hit_state: %d, desired transition: %d\n",
          RegNext(dtlb.io.req.bits.vaddr), s2_req.addr, s2_hit, s2_replay, isWrite(s2_req.cmd), s2_tag_match, s2_has_permission, s2_hit_state === s2_new_hit_state, s2_dbg_req)
      //printf("---------------------\n")
    }
  }

  /*
  when(RegNext(s1_replay && s1_replay_lookup))
  {
    printf("Addr 0x%x Data 0x%x\n", s2_req.addr, cache_resp.bits.data)
  }
  */
  ...
  io.cpu.replay_next := (s1_replay && s1_read && !(s1_replay && s1_replay_lookup)) || mshrs.io.replay_next

  io.cpu.resp.bits.paddr := s1_addr

  val s1_xcpt_valid = dtlb.io.req.valid && !s1_nack
  val s1_xcpt = dtlb.io.resp
  /*
  when(RegNext(s1_xcpt_valid)) {
    printf("D$XCPT addr: 0x%x ", s2_req.addr)
    printf("ma.st : %d  ", RegNext(s1_xcpt.ma.st))
    printf("ma.ld : %d  ", RegNext(s1_xcpt.ma.ld))
    printf("pf.st : %d  ", RegNext(s1_xcpt.pf.st))
    printf("pf.ld : %d  ", RegNext(s1_xcpt.pf.ld))
    printf("ae.st : %d  ", RegNext(s1_xcpt.ae.st))
    printf("ae.ld : %d\n", RegNext(s1_xcpt.ae.ld))
  }
  */
  io.cpu.s2_xcpt := Mux(RegNext(s1_xcpt_valid), RegEnable(s1_xcpt, s1_clk_en), 0.U.asTypeOf(s1_xcpt))

  io.cpu.perf.acquire := mshrs.io.req.fire() && !isPrefetch(s2_req.cmd)//edge.done(tl_out.a)
  io.cpu.perf.release := cache_resp.valid && !s2_req.isLookup && !isPrefetch(s2_req.cmd)
  io.cpu.perf.tlbMiss := io.ptw.req.fire()
  // Try to count misses in a different way to compare against edge.done
  // TODO actually count prefetcher induced misses
  io.cpu.perf.access  := dtlb.io.req.valid && !dtlb.io.resp.miss
  io.cpu.perf.mshr_busy := mshr_busy
}
