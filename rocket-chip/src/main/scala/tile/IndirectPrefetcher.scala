...
class IndirectPrefetcher(enablePrefetch: Boolean = true, lookupOnly: Boolean = false)(implicit p: Parameters) extends Module{
  val io = IO(new Bundle{
  ...
  val (s_idle :: s_atomlookup :: s_wait_atomlookup ::
    s_readatom :: s_check_atom :: s_pref :: s_wait_resp :: s_pref_ind :: s_wait_pref_ind :: Nil) = Enum(9)
  ...
  */
  ...
  val stride                = RegEnable(io.pat_atom(255,192), state === s_readatom)
  val ds_base_address       = RegEnable(io.pat_atom(63,0), state === s_readatom) // Find base address of the data structure 
  val chain                 = RegEnable(io.pat_atom(127,64), state === s_readatom) // Find the base of the indirect ds - > B[A[i]] -> base address of B
  val index                 = RegEnable((pref_vaddr - io.pat_atom(63,0))/4.U, state === s_readatom) // Find the index that A was accessed with
  val bounds                = RegEnable(io.pat_atom(191,128),state === s_readatom)
  val atom_enabled          = RegEnable(io.ast_status, state === s_readatom)
  val same_page             = (pref_vaddr + stride*64.U)(11) === pref_vaddr(11)
  val indirect_index        = RegEnable(io.mem.resp.bits.data(31,0), (state === s_wait_resp) && io.mem.resp.valid)
  val pref_addr             = RegEnable(pref_vaddr + io.pat_atom(255,192), state === s_readatom)
  val in_bounds             = RegEnable(((io.pat_atom(191,128) > (pref_vaddr + io.pat_atom(255,192)-io.pat_atom(63,0))/4.U)) && (pref_vaddr > io.pat_atom(63,0)) ,state === s_readatom)
  val pref_indirect         = RegEnable(chain + io.mem.resp.bits.data(31,0)*4.U, state === s_wait_resp && io.mem.resp.valid && chain != 0.U)
  ...
  io.mem.req.valid     := (state === s_pref) || (state === s_pref_ind) 
  io.mem.req.bits.tag  := UInt(0)
  //io.mem.req.bits.cmd  := Mux(state === s_pref, M_XRD,M_PFR) //M_XLR //-> read with intent to write
  io.mem.req.bits.cmd  := Mux(state === s_pref, M_XRD,M_PFR) //M_XLR //-> read with intent to write
  io.mem.req.bits.typ  := pref_typ // MT_WU:350
  io.mem.req.bits.phys := false.B
  io.mem.req.bits.addr := Mux(state === s_pref,pref_addr,pref_indirect)
  ...
  {
      printf("=========== STATE == check_atom ===========")
      printf("Atom id = %d\n", atom_id)
      printf("DATA_STRUCTURE --- base_address = %d\n", ds_base_address)
      printf("DATA_STRUCTURE --- pref_vaddr = %d\n", pref_vaddr)
      printf("STRIDE --- stride = %d\n", stride)
      printf("DATA_STRUCTURE --- bounds = %d\n", bounds)
      printf("CHAIN DS --- indirect_address: %d \n", chain)
      printf("DATA_STRUCTURE_INDEX --- index = %d \n ",index)
      printf("IN_BOUNDS %d \n ",in_bounds)
      printf("ATOM_ENABLED %d \n ",atom_enabled)
      printf("PREF ADDRESS %d \n ",pref_addr)
      printf("=========== END STATE == check_atom ===========")
      when(in_bounds && atom_enabled){
        state := s_pref
        printf("Going to prefetch\n")
      }.otherwise{
        state := s_idle
      }
  }.elsewhen(state === s_pref)
  {
      ...
      printf("ATOM_ENABLED %d \n ",atom_enabled)
      printf("PREF ADDRESS %d \n ",pref_addr)
      printf("=========== END STATE == s_pref ===========")
      
    when(io.mem.req.fire()){
      printf("Waiting for prefetcher 1\n")     
      state := s_wait_resp
      }

  }.elsewhen(state === s_wait_resp){
  
    when(io.mem.resp.valid && chain != 0.U){
      printf("=========== STATE == s_wait_resp ===========")
      printf("Atom id = %d\n", atom_id)
      printf("DATA_STRUCTURE --- base_address = %d\n", ds_base_address)
      printf("DATA_STRUCTURE --- pref_vaddr = %d\n", pref_vaddr)
      printf("STRIDE --- stride = %d\n", stride)
      ...
      printf("=========== END STATE == s_wait_resp ===========")
      state := s_pref_ind
      printf("No chain \n");
    }.elsewhen(io.mem.resp.valid  && chain === 0.U){
      state := s_idle
    }
    .elsewhen(io.mem.s2_nack){
      state := s_idle
    }
  }.elsewhen(state === s_pref_ind)
  {
      printf("=========== STATE == s_pref_ind ===========")
      printf("Atom id = %d\n", atom_id)
      printf("DATA_STRUCTURE --- base_address = %d\n", ds_base_address)
      printf("DATA_STRUCTURE --- pref_vaddr = %d\n", pref_vaddr)
      printf("STRIDE --- stride = %d\n", stride)
      ...
      printf("PREF ADDRESS %d \n ",pref_addr)
      printf("INDIRECT INDEX %d \n ",indirect_index)
      printf("PREFETCH INDIRECT %d \n ",pref_indirect)
      printf("=========== END STATE == s_pref_ind ===========")
    when(io.mem.req.fire()){
      state := s_idle
    }
...
class Prefetcher(albSize: Int = 32, atomGranularity: Int = 20)(implicit p: Parameters) extends LazyRoCC {
  override lazy val module = new IndirectPrefetcherModule(this, albSize, atomGranularity)
  override val atlNode = TLClientNode(Seq(TLClientPortParameters(Seq(TLClientParameters("Prefetcher")))))
}

class IndirectPrefetcherModule (outer: Prefetcher,
  ...
  val pref = Module(new IndirectPrefetcher())
