...
case class RoCCParams(
  ...
  disableCommandRouter: Boolean = false)
...
class CoreASPIO extends Bundle{
  ...
  val addr     = UInt(40.W).asOutput
  val typ      = UInt(3.W).asOutput
  val cmd      = UInt(5.W).asOutput
  val s1_paddr = UInt(40.W).asOutput
  val s1_kill  = Bool(OUTPUT)
  val s2_nack  = Bool(OUTPUT)
  val s2_xcpt  = (new HellaCacheExceptions).asOutput
  val physical = Bool(OUTPUT)
  val miss     = Bool(OUTPUT)
  val bp_q_full= Bool(INPUT)
  val bc_resolved = Bool(INPUT)
...
trait HasLazyRoCC extends CanHavePTW { this: BaseTile =>
  ...
  // TODO Temporary fix to connect prefetcher embedded in the accelerator to dmem
  nDCachePorts += (roccs.size*3)
}
...
trait HasLazyRoCCModule[+L <: BaseTile with HasLazyRoCC] extends CanHavePTWModule
  ...
  val breakDesign = usingRocc && buildRocc(0).disableCommandRouter
  ...
  if(usingRocc) {
    ...
    if(breakDesign)
    {
      ...
      outer.roccs.zipWithIndex.foreach { case (rocc, i) =>
        ...
        rocc.module.io.cmd <> roccCore.cmd//cmdRouter.io.out(i)
        rocc.module.io.core_snoop <> roccCore.core_snoop
        rocc.module.io.ptw_snoop <> ptw.io.snoop
        rocc.module.io.exception := roccCore.exception
        val dcIF = Module(new SimpleHellaCacheIF()(outer.p))
        val dcIF_pref = Module(new SimpleHellaCacheIF()(outer.p))
        dcIF.io.requestor <> rocc.module.io.mem
        // dcIF_pref.io.requestor <> rocc.module.io.mem_pref
        println("Before adding dcIF" + dcachePorts.size)
        dcachePorts += dcIF.io.cache
        // dcachePorts += dcIF_pref.io.cache
        println("Before adding prefetcher" + dcachePorts.size)
        dcachePorts += rocc.module.io.mem_pref
        println("Before adding amu_lookup" + dcachePorts.size)
        dcachePorts += rocc.module.io.mem_amulookup
