...
class IndirectPrefetcherStreamOnly(enablePrefetch: Boolean = true, lookupOnly: Boolean = false)(implicit p: Parameters) extends Module{
  val io = IO(new Bundle{
  ...
  val (s_idle :: s_atomlookup :: s_wait_atomlookup ::
    s_readatom :: s_check_atom :: s_pref :: s_wait_resp :: s_pref_ind :: s_wait_pref_ind :: s_finish:: Nil) = Enum(10)
  ...
  */
  ...
  val pref_indirect         = RegEnable(chain + indirect_index*4.U, state === s_pref_ind)
  ...
  io.mem.req.bits.cmd  := Mux(state === s_pref, M_PFR,M_PFR) //M_XLR //-> read with intent to write
  ...
  }.elsewhen(state === s_wait_pref_ind)    
  {
...
class PrefetcherStreamOnly(albSize: Int = 32, atomGranularity: Int = 20)(implicit p: Parameters) extends LazyRoCC {
  override lazy val module = new IndirectPrefetcherStreamOnlyModule(this, albSize, atomGranularity)
  override val atlNode = TLClientNode(Seq(TLClientPortParameters(Seq(TLClientParameters("Prefetcher")))))
}

class IndirectPrefetcherStreamOnlyModule (outer: PrefetcherStreamOnly,
  ...
  val pref = Module(new IndirectPrefetcherStreamOnly())
