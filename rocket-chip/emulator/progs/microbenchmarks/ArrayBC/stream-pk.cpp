...
int main()
{
  ...
  uint32_t mapSize = (arrSize-128)/(1<<GRANULARITY) == 0 ? 1 : arrSize/(1<<GRANULARITY);
  ...
  #ifndef NOATOM
  #ifndef LOOKUPONLY
  atom_map((void*) (b+16), mapSize, 1);
  atom_map((void*) (c+16), mapSize, 2);
  atom_map((void*) (a+16), mapSize, 3);
  #endif
  #endif
  for(int iter = 0 ; iter < 20 ; iter++)
    for(i = 16 ; i < arrSize/4 - 16 ; i += 1)
    {
      atom_select(1);
