...
SecurePointer<node_t> **make_table(int size, int procname) {
  SecurePointer<node_t> **retval = (SecurePointer<node_t> **)malloc(size*sizeof(SecurePointer<node_t> *)) ;
  for(int i=0;i<size;i++){
    retval[i] = new SecurePointer<node_t>(sizeof(node_t));
  }
  //= (SecurePointer<node_t> **)malloc(size*sizeof(node_t *));
  assert(retval);
  return retval;
}

/* We expect node_table to be a local table of e or h nodes */
void fill_table(SecurePointer<node_t> **node_table, long *values, int size, int procname)
{
  SecurePointer<node_t> *cur_node;
  SecurePointer<node_t> *prev_node = new SecurePointer<node_t>(sizeof(node_t));
  int i;

//  prev_node = (node_t *)malloc(sizeof(node_t));
  node_table[0] = prev_node;
  *values = gen_uniform_long();
  (*prev_node)->value = values++;
  (*prev_node)->from_count = 0;

  /* Now we fill the node_table with allocated nodes */
  for (i=1; i<size; i++) {
    cur_node = new SecurePointer<node_t>(sizeof(node_t));
    *values = gen_uniform_long();
...
void make_neighbors(SecurePointer<node_t> *nodelist, SecurePointer<node_t> **table[], int tablesz,
		    int degree, int percent_local, int id)
{
  SecurePointer<node_t> *cur_node;

  for(cur_node = nodelist; cur_node; cur_node=(*cur_node)->next) {
    SecurePointer<node_t> *other_node;
    int j,k;
    int dest_proc;

    (*cur_node)->to_nodes = ( SecurePointer<node_t> **)malloc(degree*(sizeof(SecurePointer<node_t> *)));
    for (int i =0;i<degree;i++) {
      (*cur_node)->to_nodes[i]= new SecurePointer<node_t>(sizeof(node_t));
    }
    ...
    for (j=0; j<degree; j++) {
      do {
        SecurePointer<node_t> **local_table;
...
void update_from_coeffs(SecurePointer<node_t> *nodelist) {
  SecurePointer<node_t> *cur_node;
  ...
  for (cur_node = nodelist; cur_node; cur_node=(*cur_node)->next) {
    ...
    if (from_count < 1) {
      ...
      (*cur_node)->from_values = (long **)malloc(20 * sizeof(long *));
      (*cur_node)->coeffs = (long *)malloc(20 * sizeof(long));
      (*cur_node)->from_length = 0;
    } else {
      (*cur_node)->from_values = (long **)malloc(from_count * sizeof(long *));
      (*cur_node)->coeffs = (long *)malloc(from_count * sizeof(long));
      (*cur_node)->from_length = 0;
    }
  }
}

void fill_from_fields(SecurePointer<node_t> *nodelist, int degree) {
  SecurePointer<node_t> *cur_node;
  for(cur_node = nodelist; cur_node; cur_node = (*cur_node)->next) {
    int j;
    ...
    for (j=0; j<degree; j++) {
      int count,thecount;
      SecurePointer<node_t> *other_node = (*cur_node)->to_nodes[j]; /* <-- 6% load miss penalty */
      long **otherlist;
      long *value = (*cur_node)->value;
      ...
      (*other_node)->coeffs[count]=gen_uniform_long();
    }
  }
}

void localize_local(SecurePointer<node_t> *nodelist) {
  SecurePointer<node_t> *cur_node;
...
void make_tables(SecurePointer<table_t> *table,int groupname) {
  SecurePointer<node_t> **h_table,**e_table;
  long *h_values, *e_values;
  int procname = 0;
  chatting("make_tables\n");
  init_random(SEED1*groupname);
  h_values = (long *)malloc(n_nodes/PROCS*sizeof(long));
  h_table = make_table(n_nodes/PROCS,procname);
  fill_table(h_table,h_values,n_nodes/PROCS,procname);
  e_values = (long *)malloc(n_nodes/PROCS*sizeof(long));
...
void make_all_neighbors(SecurePointer<table_t> *table,int groupname) {
  SecurePointer<node_t> *first_node;
  SecurePointer<node_t> **local_table;
  SecurePointer<node_t> ***local_table_array;
...
void update_all_from_coeffs(SecurePointer<table_t> *table, int groupname)
{
  SecurePointer<node_t> **local_table;
  SecurePointer<node_t> *first_node;
...
void fill_all_from_fields(SecurePointer<table_t> *table, int groupname)
{
  SecurePointer<node_t> **local_table;
  SecurePointer<node_t> *first_node;
...
void localize(SecurePointer<table_t> *table, int groupname)
{
  SecurePointer<node_t> **local_table;
  SecurePointer<node_t> *first_node;
...
void clear_nummiss(SecurePointer<table_t> *table, int groupname)
{
  NumMisses = 0;
}

void do_all(SecurePointer<table_t> *table, int groupname, int nproc,
	    void func(SecurePointer<table_t> *, int),int groupsize) {
...
SecurePointer<graph_t> *initialize_graph() {
  SecurePointer<table_t> *table = new SecurePointer<table_t>(sizeof(table_t));
  SecurePointer<graph_t> *retval = new SecurePointer<graph_t>(sizeof(graph_t));
  int i,j,blocksize;
  int groupsize;
  ...
  for (i=0; i<NumNodes; i++) {
    SecurePointer<node_t> **local_table = (*table)->e_table[i*blocksize];
    SecurePointer<node_t> *local_node_r = local_table[0];
    ...
    for (j = 1; j < blocksize; j++) {
      SecurePointer<node_t> *local_node_l;
