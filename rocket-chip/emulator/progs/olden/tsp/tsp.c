...
static long distance(SecurePointer<Tree>& a, SecurePointer<Tree>& b) {
  long ax,ay,bx,by;
...
static SecurePointer<Tree>* makelist(SecurePointer<Tree>* t) {
  SecurePointer<Tree> *left, *right;
  SecurePointer<Tree> *tleft, *tright;
  SecurePointer<Tree> *retval = t;
...
static void reverse(SecurePointer<Tree>* t) {
  SecurePointer<Tree> *prev,*back,*next,*tmp;
...
SecurePointer<Tree>* conquer(SecurePointer<Tree>* t) {
  SecurePointer<Tree> *cycle,*tmp,*min,*prev,*next,*donext;
  long mindist,test;
...
static SecurePointer<Tree>* merge(SecurePointer<Tree> *a, SecurePointer<Tree> *b, SecurePointer<Tree> *t, int nproc) {
  SecurePointer<Tree> *min,*next,*prev,*tmp;
  long mindist,test,mintonext,mintoprev,ttonext,ttoprev;
  SecurePointer<Tree> *n1,*p1,*n2,*p2;
...
SecurePointer<Tree>* tsp(SecurePointer<Tree>* t,int sz,int nproc) {
  SecurePointer<Tree>* left,*right;
  SecurePointer<Tree>* leftval;
#ifdef FUTURES
  future_cell_pointer fc;
#endif
  SecurePointer<Tree>* rightval;
  int nproc_2 = nproc/2;
