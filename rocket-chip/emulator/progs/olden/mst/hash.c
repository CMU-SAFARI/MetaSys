...
void *HashLookup(unsigned int key, Hash hashArg)
{
  int j;
  SecurePointer<struct hash_entry> ent;
  SecurePointer<struct hash> hash(hashArg, sizeof(struct hash));

  j = (hash->mapfunc)(key);        /* 14% miss in hash->mapfunc */  
  assert(1,j>=0);
  assert(2,j<hash->size);
  for (ent = SecurePointer<struct hash_entry>(hash->array[j],sizeof(struct hash_entry));       /* 17% miss in hash->array[j] */ /* adt_pf can't detect :( */
       !(ent == NULL) &&                      /* 47% miss in ent->key */       /* adt_pf can detect :) */
           ent->key!=key; 
       ent= SecurePointer<struct hash_entry>(ent->next,sizeof(struct hash_entry)));             /* 8% miss in ent->next */       /* adt_pf can detect :) */
