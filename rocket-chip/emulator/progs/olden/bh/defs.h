...
typedef struct bnode {
    ...
    SecurePointer<bnode>* next;              /* pointer to next point */
    SecurePointer<bnode>* proc_next;              /* pointer to next point */
} body;
...
typedef struct cnode {
    ...
    struct node * next_few_node;
#endif
    SecurePointer<node>* subp[NSUB];         /* descendents of cell */
    SecurePointer<cnode>* next;               /* for free list */
...
typedef struct {
  ...
  SecurePointer<bnode>* bodytab[MAX_NUM_NODES];
  SecurePointer<bnode>* bodiesperproc[MAX_NUM_NODES];
} tree, *treeptr;
...
template class SecurePointer<tree>;
template class SecurePointer<node>;
template class SecurePointer<body>;
template class SecurePointer<cell>;
...
#define eps 5 		/* potential softening parameter            */
#define tol 100 		/* accuracy parameter: 0.0 => exact         */
#define NSTEPS  10
#define fcells 750		/* ratio of cells/bodies allocated          */
