...
Demand Compute_Branch(SecurePointer<Branch> &br, long theta_R, long theta_I, 
                       long pi_R, long pi_I) {
  Demand a2,tmp;
  long new_pi_R, new_pi_I;
  long a,b,c,root;
  SecurePointer<Leaf> *l;
  SecurePointer<Branch> *next;
  int i;
  Demand a1;
...
void Compute_Tree(SecurePointer<Root>* r) {
  ...
  for (i=0; i<NUM_FEEDERS; i++) {
    theta_R = (*r)->theta_R;
    theta_I = (*r)->theta_I;
    a = Compute_Lateral(*(((*r)->feeders)[i]),theta_R,theta_I,theta_R,theta_I);
    tmp.P += a.P;
    tmp.Q += a.Q;
  }
  (*r)->D.P = tmp.P;
